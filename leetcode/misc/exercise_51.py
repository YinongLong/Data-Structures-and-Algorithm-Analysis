...
class Node(object):
    ...
    def __init__(self, path, parent, positions, level):
        """
        :type x: int
            生成该结点的x坐标
        :type y: int
    ...
    def solveNQueens(self, n):
        ...
        while temp_node:
            if temp_node.level == n:  # 说明n个位置都已经放置棋子，则返回结果
                solutions.append(temp_node.path)
                temp_node = temp_node.parent
                continue
            if len(temp_node.positions) == 0:  # 代表该结点不能再生成子结点
                temp_node = temp_node.parent
            else:  # 代表该结点还可以生成子结点
                (next_x, next_y), children = temp_node.next()
                temp_node = Node(temp_node.path + [(next_x, next_y)], temp_node, children, temp_node.level + 1)
